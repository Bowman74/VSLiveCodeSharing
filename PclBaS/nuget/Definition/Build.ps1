...
function ChangeNuSpecVersion( $nuSpecFilePath, $version="0.0.0.0" )
{
    ...
    foreach( $dependency in $dependencies )
    {
        ...
        {
            if ( $idAttribute.Value -eq "Demo Library" )
            {
...
try 
{
    ...
    $pathToBin = [System.IO.Path]::GetFullPath( "$basePath\..\..\TestLibrary.Android\bin\Debug" )
    ...
    ## ----------------------    
    ## Before building NuGet package, extract  Version number and update .NuSpec to automate versioning of .NuSpec document
    ...
    $cslaAssembly = Get-ChildItem "$pathToBin\TestLibrary.dll" | Select-Object -First 1
